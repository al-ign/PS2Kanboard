<#
.Synopsis
   Get the role of a user for a given project
...
.DESCRIPTION
   ...
   Alias: Invoke-KBGetProjectUserRole
.OUTPUTS
   Returns role name on success
   Returns false on failure
.NOTES
   API Function Name: getProjectUserRole
   PS Module Safe Name: Invoke-KBGetProjectUserRole
   Function parsed from: ProjectPermissionProcedure.php
   Description parsed from: project_permission_procedures.rst
#>
function Get-KBProjectUserRole {
...
Begin {
$ApiParameters = @('project_id', 'user_id')
$hashApiParameters = @{}
...
$jsonParameters = @('project_id', 'user_id')
$hashjsonParameters = @{}
...
} # End Get-KBProjectUserRole function
