...
.DESCRIPTION
   ...
   Alias: Invoke-KBCreateProjectFile
.OUTPUTS
   Returns file_id on success
   Returns false on failure
.NOTES
   API Function Name: createProjectFile
   PS Module Safe Name: Invoke-KBCreateProjectFile
   Function parsed from: ProjectFileProcedure.php
   Description parsed from: project_file_procedures.rst
#>
function New-KBProjectFile {
...
Begin {
$ApiParameters = @('project_id', 'filename', 'blob')
$hashApiParameters = @{}
...
$jsonParameters = @('project_id', 'filename', 'blob')
$hashjsonParameters = @{}
...
} # End New-KBProjectFile function
