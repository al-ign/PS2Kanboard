<#
.Synopsis
   Move a task to another column, position or swimlane inside the same board

.DESCRIPTION
   -  Purpose: Move a task to another column, position or swimlane inside
      the same board
   -  Parameters:
      -  project_id (integer, required)
      -  task_id (integer, required)
      -  column_id (integer, required)
      -  position (integer, required)
      -  swimlane_id (integer, required)
   -  Result on success: true
   -  Result on failure: false
   Alias: Invoke-KBMoveTaskPosition, Move-KBTaskPosition
.OUTPUTS
   Returns true on success
   Returns false on failure
.NOTES
   API Function Name: moveTaskPosition
   PS Module Safe Name: Invoke-KBMoveTaskPosition
   Function parsed from: TaskProcedure.php
   Description parsed from: task_procedures.rst
#>
function Set-KBTaskPosition {
[CmdletBinding(DefaultParameterSetName='PlainCredentials')]
[Alias('Invoke-KBMoveTaskPosition', 'Move-KBTaskPosition')]
Param (
    [Parameter(Mandatory=$true)]
    [int]$project_id,

    [Parameter(Mandatory=$true)]
    [int]$task_id,

    [Parameter(Mandatory=$true)]
    [int]$column_id,

    [Parameter(Mandatory=$true)]
    [int]$position,

    [Parameter(Mandatory=$true)]
    [int]$swimlane_id,

    # Credential Object
    [Parameter(Mandatory=$true, ParameterSetName='CredentialObject')]
    $Credential,

    # Kanboard API Uri
    [Parameter(Mandatory=$true, ParameterSetName='PlainCredentials')]
    [string]$ApiUri,

    # API Username, use "jsonrpc" for the global access
    [Parameter(Mandatory=$true, ParameterSetName='PlainCredentials')]
    [string]$ApiUsername,

    # API Password or Token
    [Parameter(Mandatory=$true, ParameterSetName='PlainCredentials')]
    [Alias('Token')]
    [string]$ApiPassword
    )
Begin {
$ApiParameters = @('project_id', 'task_id', 'column_id', 'position', 'swimlane_id')
$hashApiParameters = @{}

foreach ($par in $hashApiParameters) {
    if ($PSBoundParameters.Keys -contains $par) {
        $hashApiParameters.Add($par, $PSBoundParameters[$par])
        }
    }

$jsonParameters = @('project_id', 'task_id', 'column_id', 'position', 'swimlane_id')
$hashjsonParameters = @{}

foreach ($par in $jsonParameters) {
    if ($PSBoundParameters.Keys -contains $par) {
        $hashjsonParameters.Add($par, $PSBoundParameters[$par])
        }
    }

if ($PSCmdlet.ParameterSetName -eq 'PlainCredentials') {
    $Credential = New-KanboardCredential $ApiUri $ApiUsername $ApiPassword
    }
} # End begin block

End {
$jsonRequestId = Get-Random -Minimum 1 -Maximum ([int]::MaxValue)

$json = @{
    jsonrpc = '2.0'
    method = 'moveTaskPosition'
    id = $jsonRequestId        
    }

# dynamically add user parameters
if ($hashJsonParameters.Count -gt 0) {
    $json.Add('params', $hashJsonParameters)
    }

$json = $json | ConvertTo-Json
    
if ($PSBoundParameters['Verbose']) {
    Write-Verbose $json
    }

$splat = @{
    Method = 'POST'
    Uri = $Credential.Uri
    Credential = $Credential.Credential
    Body = $json
    ContentType = 'application/json'
    }

$res = Invoke-RestMethod @splat

if ($res.result) {
    Convert-KanboardResult $res.result -PassThru
    }

} # End end block


} # End Set-KBTaskPosition function

